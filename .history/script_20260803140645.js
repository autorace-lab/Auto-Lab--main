...
Object.assign(race, data.raceInfo);
...
const venue =
titleText.match(/(川口|伊勢崎|浜松|山陽|飯塚)オート/)?.[0] || "";

const raceNo =
doc.querySelector(".live-mainTab01__item--current")
?.dataset.raceno + "R";

const raceTitle =
doc.querySelector("h3.h4")?.textContent.trim() || "";
...
const infoTables =
doc.querySelectorAll(".race-infoTable");
...
const weatherCells =
infoTables[0].querySelectorAll("tbody td");

const conditionCells =
infoTables[1].querySelectorAll("tbody td");


weather = weatherCells[3]?.innerText.trim() || "";

trackTemp = conditionCells[2]?.innerText.trim() || "";

track = conditionCells[3]?.innerText.trim() || "";
