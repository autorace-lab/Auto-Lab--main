...
// =========================
const finalRaceNo = Number(data.raceInfo?.finalRaceNo ?? data.finalRaceNo ?? 12);
...
try {
    ...
    if(!response.ok){
        throw new Error(
            `邨先棡JSON蜿門ｾ怜､ｱ謨・ ${response.status}`
        );
    }
...
track:
    ...
    finalRaceNo: Number(data.raceInfo?.finalRaceNo ?? data.finalRaceNo ?? 12)
...
async function addALVerificationRecord(resultList){
    ...
    if(!newData.length){
        console.error("讀懆ｨｼ繝・・繧ｿ繧剃ｽ懈・縺ｧ縺阪∪縺帙ｓ");
        return [];
    }
